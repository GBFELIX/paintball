    ...
    const confirmCloseGame = async () => {
        ...
        try {
            ...
            const promises = Object.keys(storedItems).map(async (itemName) => {
                ...
                const urlSafeItemName = itemName.replace(/ /g, '%20');
                const response = await axios.get(`/.netlify/functions/api-estoque/${urlSafeItemName}`);
                ...
                await axios.put(`/.netlify/functions/api-estoque/${urlSafeItemName}`, { nome: itemName, quantidade: novaQuantidade });
